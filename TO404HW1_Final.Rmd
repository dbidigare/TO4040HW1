...
```
**One January 12th, 2013, there were `r jan_12_2013_UAFlights` United Airlines flights.**
...
```{r}
average_arrival_delay_Jan2013 <- mean(nyc$arr_delay[nyc$year==2013 & nyc$month==1], na.rm=TRUE)
...
```
**`r best_airgain` are the airlines did the best job gaining airtime, while `r worst_airgain` are the airlines that did the worst job gaining airtime.**
